...
from cowrie.utils.db_utils import log_directory_creation
from cowrie.utils.db_utils import validate_directory
from cowrie.utils.db_utils import remove_logged_directory
import time
...
class Command_mkdir(HoneyPotCommand):
    ...
    def call(self) -> None:
        for f in self.args:
            pname = self.fs.resolve_path(f, self.protocol.cwd)
            session_id = self.protocol.terminal.transport.session.id
            ...
            try:
                ...
                log.msg(f"DEBUG: Directory '{pname}' created in memory.")

                # Log directory creation
                log_directory_creation(session_id, pname)

            except Exception as e:
                log.err(f"ERROR: Failed to create directory '{pname}': {e}")
...
class Command_rmdir(HoneyPotCommand):
    ...
    def call(self) -> None:
        for f in self.args:
            ...
            if not validate_directory(session_id, pname):
                self.errorWrite(f"rmdir: failed to remove `{f}': Permission denied\n")
                continue

            try:
                # Check if directory is empty
                if len(self.fs.get_path(pname)):
                    self.errorWrite(f"rmdir: failed to remove `{f}': Directory not empty\n")
                    continue

                # Remove directory in memory
                self.fs.rmdir(pname)
                log.msg(f"DEBUG: Directory '{pname}' removed in memory.")

                # Remove directory from logs
                remove_logged_directory(session_id, pname)

            except Exception as e:
                log.err(f"ERROR: Failed to remove directory '{pname}': {e}")
                self.errorWrite(f"rmdir: failed to remove `{f}': {e}\n")
