...
from cowrie.utils.db_utils import get_session_id
...
class UserDB:
    ...
    def checklogin(
        ...
        for credentials, policy in self.userdb.items():
            ...
            if self.match_rule(login, thelogin) and self.match_rule(passwd, thepasswd):
                ...
                self.replay_directories(thelogin.decode("utf8"), thepasswd.decode("utf8"), src_ip)
    ...
    def replay_commands(self, username: str, password: str, ip: str) -> None:
        ...
        """
         # Get session ID for the user, password, and IP
        session_id = get_session_id(self.db, username, password, ip)
        if not session_id:
            log.msg(f"No valid session found for {username}@{ip}. Skipping command replay.")
            return
        
        # Retrieve the session associated with the user and ip
        query = """
        ...
        try:
            ...
            if not past_commands:
                log.msg(f"No past commands to replay for {username}@{ip}.")
                return
    ...
    def replay_directories(self, username: str, password: str, ip: str) -> None:
        """
        ...
        try:
            ...
            for command in past_directories:
                ...
                # Re-run the mkdir command in the session
                self.shell.execute_command(command[0])  # Adjust depending on your session's exec method
        except Error as e:
