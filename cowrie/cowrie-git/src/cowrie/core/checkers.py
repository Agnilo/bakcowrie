...
class HoneypotPasswordChecker:
    ...
    def requestAvatarId(self, credentials):
        ...
        log.msg(f"Passing protocol: {protocol} for user: {credentials.username}")

        if hasattr(credentials, "password"):
            if self.checkUserPass(credentials.username, credentials.password, credentials.ip, protocol):
                return defer.succeed(credentials.username)
    ...
    def checkUserPass(self, theusername: bytes, thepassword: bytes, ip: str, protocol=None) -> bool:
        ...
        log.msg(f"Protocol passed: {protocol} for user: {theusername.decode('utf-8')}")
